...
namespace Luna_old {
	...
	export namespace Event {
		...
		export interface ListenerEntry<T extends keyof PayloadMap> {
			...
			callback: (event: TypeConstructor<T, PayloadMap[T]>) => unknown
		}

		/** List of all event listeners */
		const events: Map<keyof PayloadMap, ListenerEntry<keyof PayloadMap>[]> = new Map()
		 ...
		 * @returns Listener ID
		 */
		export function on<T extends keyof PayloadMap>(event: T, callback: (event: TypeConstructor<T, PayloadMap[T]>) => unknown) {
			const arr = events.get(event) ?? []
			const entry = { callback, id: Internal.gen_id() } as ListenerEntry<typeof event>
	...
	export namespace Class {
		...
		export class Range {
			...
			public static from(str: string) {
				...
				const raw_min = /^!?\d+?(?=-!?\d+?$)/.exec(str)[0]
				const raw_max = /(?<=^!?\d+?-)!?\d+?$/.exec(str)[0]

				const min = +raw_min.replace("!", "")
				const max = +raw_max.replace("!", "")
				const eql_min = !raw_min.startsWith("!")
				const eql_max = !raw_max.startsWith("!")
			...
			public get_random() {
				...
				const max = this.max.val + +this.max.eql
				const mod = max - min
		...
		export class Vector2D {
			...
			public to_3d() {
				const z = this instanceof Vector3D ? this.z : 0
	...
	export namespace Display {
		...
		export interface Renderable {
			 ...
			 */
			render(delta: number, ctx: CanvasRenderingContext2D): Promise<unknown>
		}
	...
	let fps: number[] = []
	/** Internal settings */
	const settings = { log_fps: true, log_interval: 30 }
	...
	export async function request_frame(vsync = true) {
		...
		if (vsync) {
			...
			await process(performance.now(), vsync)
		}
	}
	...
	export async function process(time: number, vsync = true) {
		...
		if (settings.log_fps) {
			if (frame.last !== 0) fps.push(delta)
