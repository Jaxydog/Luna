...
(function (Shorthand) {
    ...
    (function (Lexer) {
        ...
        function reverse_map_get(map, value) {
        ...
        function tokenize(input) {
            ...
            let char = next_char();
            while (char) {
                let token = Lexer.Tokens.get(char);
                token ??= `NTC_${char}`;
                output.push(token);
                char = next_char();
            }
        ...
        function remove_comments(input) {
            ...
            for (let token of input) {
                if (token === "OCTOTHORPE") {
                    ...
                    if (!comment)
                        output.push(token);
                    else if (token === "BS_NEWLINE" || token === "BS_CARRIAGERETURN")
                        comment = false;
                }
            }
            return output;
        ...
        function clump(input) {
            let source = [undefined, ...input];
            let output = [];
            while (input.length !== 0) {
                let clumped;
                for (let key of Lexer.Clumps.keys()) {
                ...
                if (clumped) {
                    ...
                    output.push(Lexer.Clumps.get(clumped));
                    input.slice(0, size);
        ...
        Lexer.resolve_vars = resolve_vars;
        function resolve_strs(input) {
            let output = [];
            let str = "";
            let parsing = false;
            let str_type;
            for (let token of input) {
                if (!parsing) {
                    ...
                    if (token === "SINGLEQUOTE" && str_type === "s") {
                        ...
                        let translated = reverse_map_get(Lexer.Tokens, token);
        ...
        Lexer.resolve_strs = resolve_strs;
        function remove_br_sp(input) {
            return input.filter((v) => !v.startsWith("BS_"));
        }
        Lexer.remove_br_sp = remove_br_sp;
        function resolve_lbls(input) {
            let output = [];
            let parsing = false;
            let label = "";
            for (let token of input) {
        ...
        Lexer.resolve_lbls = resolve_lbls;
        function lex(input) {
